    ...
    try {
        ...
        if($db->num_rows($consulta) != 0)
        {
            while($resultados = $db->fetch_array($consulta))
            {
                ...
                $cadena_datos .= '<button type="button" id="btnModificar" data-theme="b" onclick="TiposSeguimientosModificarTipoSeguimiento(' . $resultados['IdTipoSeguimiento'] .')" class="ui-btn ui-shadow ui-corner-all ui-btn-b ui-btn-icon-left ui-icon-edit">Modificar</button>';
                $cadena_datos .= '</div>';
                $cadena_datos .= '<div class="col-xs-1"></div>';
                $cadena_datos .= '</div>';
            }
